...
def evolution(request, course_id, student_id):
    # Obtener el curso y el estudiante
    course = get_object_or_404(Course, id=course_id)
    ...
    # Obtener estado de inscripción del estudiante en el curso
    ...
    # Formatear datos de asistencias y notas para el PDF
    attendances_data = [
        {
    ...
    if request.GET.get('generate_pdf') == 'true':
        ...
        # Contenido del PDF
        elements.append(Paragraph(f"Evolución de {student.first_name} {student.last_name}", styles['Title']))
        elements.append(Paragraph(f"Curso: {course.name}", styles['Normal']))
        elements.append(Paragraph(f"Profesor: {teacher}", styles['Normal']))

        status_map = {'P': 'CURSO EN PROGRESO', 'I': 'EN ETAPA DE INSCRIPCION', 'F': 'CURSO FINALIZADO'}
        elements.append(Paragraph(f"Estado del Curso: {status_map.get(course.status, 'Desconocido')}", styles['Normal']))

        reg_status = "Alumno Regular" if registration_status and registration_status['enabled'] else "Alumno Irregular"
        elements.append(Paragraph(f"Condición del Estudiante: {reg_status}", styles['Normal']))

        elements.append(Spacer(1, 12))
        elements.append(Paragraph("Evaluaciones:", styles['Heading1']))
        # Tabla de notas
        marks_data = marks.first() if marks.exists() else None
        data = [["Evaluación", "Nota"]] + [
            [f"Evaluación {i+1}", getattr(marks_data, f"mark_{i+1}", "None")] for i in range(3)
        ] + [["Promedio", marks_data.average if marks_data else "None"]]
        table = Table(data)
        ...
        elements.append(Spacer(1, 12))
        elements.append(Paragraph("Asistencias:", styles['Heading1']))
        # Tabla de asistencias
        data = [["Fecha", "Asistencia"]] + [[attendance['date'], "P" if attendance['present'] else "A"] for attendance in attendances_data]
        data.append(["Total de Clases:", f"{class_quantity} - Asistencias: {present_attendance_count} - Ausencias: {absent_attendance_count}"])
        table = Table(data)
        table.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.lightgrey),
                                  ('TEXTCOLOR', (0, 0), (-1, 0), colors.black),
                                  ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
        ...
        # Si no se solicita la generación del PDF, devolver datos JSON
        evolution_data = {
